...
**Sizing philosophy.** Storage has steeply diminishing returns: most of a "fully self-sufficient" tank exists only to bridge the rarest drought. So when a configuration falls short, the verdict quantifies the gap instead of calling those years failures, and it reports two separate things. First, what bridging it from the tap would cost — priced at an assumed €4/m³, flagged as an assumption in the copy, because mains tariffs vary several-fold between countries and the volumes involved are unintuitive. Second, **which constraint is actually binding**, which is not the same question everywhere. Where a year's catchment already exceeds a year's demand the shortfall is a *storage* limit — the water lands, just not when the garden wants it — and the app solves for the smallest tank that would have carried every year on record. Where the roof does not collect enough water in a year at all, no tank can store what never falls, and the honest answer is more roof, a smaller garden, or thriftier planting. Demand-side measures (shade cloth, crop timing, soil improvement) come before extra storage either way.
